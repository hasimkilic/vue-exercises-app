<template>
    <div class="login_register_container">
      ...
      <input autofocus v-model="userData.title" type="text" placeholder="Başlık" class="input mb-3" />
      ...
      <div class="flex items-center justify-end gap-x-1">
        <router-link :to="{name:'home'}" class="secondary-button">İptal</router-link>
...
export default {
  ...
  mounted() {
    this.$appAxios.get("/categories").then(category_res => {
      this.categoryList = category_res?.data || [];
    });
  },
  methods: {
    onSave(){
      ...
      this.$appAxios.post("/bookmarks",saveData).then(save_boomark_res =>{
        console.log(save_boomark_res)
